...
//          "mantle-default-tab-background",
  ...
  setTabSelected: function(index, selected) {
    ...
    switch (selected) {
      case true:
        ...
        if (component) Displayed.show(component);
        break;
      case false:
        rCls(tabButton, "pentaho-tabWidget-selected");
        if (component) Displayed.hide(component);
        break;
    }
  },
  ...
  setSelectedTab: function(index){
    ...
    if (this.hasTab(this.selectedTab)) data.oldTab = this.selectedTab;
    if (this.fireEvent("beforeSelectTab", data) === false) return;
    if (iInt(this.selectedTab)) this.setTabSelected(this.selectedTab, false);
    if (iInt(index)) this.setTabSelected(index, true);
  ...
  closeTab: function(index) {
    ...
    if (this.fireEvent("beforeCloseTab", data) === false) return;
  ...
  addTab: function(conf) {
    ...
    var content = [];
    if (conf.text) content.push(cEl("span", {
      "class": "pentaho-tabWidgetLabel"
    }, conf.text));
    if (conf.closeable === true) content.push(
      cEl("span", {
    ...
    if (component) tabContainer.appendChild(component);
    if (conf.selected === true) this.setSelectedTab(index);
    else this.setTabSelected(index, false);
  }
};
